...
public class Cine {
	...
	private void generarAsientos() {
		// la key de cada registro del hashtable ser� la concatenaci�n de fila y columna
		// el valor ser� true por defecto, para indicar que el sitio est� disponible
		// accederemos a una posici�n as�: cine.getAsientos().get("1A");
	...
	public void sentarEspectadores(ArrayList<Espectador> espectadores) {
		for (Espectador e : espectadores) {
			...
			if (e.getEdad() >= pelicula.getEdadMin() && e.getDinero() >= precio) {
				// Generar asiento aleatorio. Si esta ocupado, genera otro hasta que encuentre
				// uno libre
				String asiento;
				do {
					asiento = "" + (int) (Math.random() * FILAS + 1) //queremos que genere filas del 1 al 8 incluidos
							...
							+ (char) (65 + ((int) ((Math.random() * (COLUMNAS - 1)) + 1)));  
